...
剧本正文自动保存，生成分镜以保存返回的完整正文版本为准；估算/结构及常驻保存、恢复、补充要求已移除，异常恢复仍保留草稿。AI文本生成进度显示在实际输出框内；建议与新故事批次不覆盖旧正文。运行入口仍为5180，事实、测试边界和简明试用步骤见[剧本与生成状态记录](docs/engineering/script-inline-generation-verification.md)。新增费用0元，无schema迁移，最终视觉/用户全链仍待验收。

## 紧凑侧栏与分镜工作台

左侧导航可收起为图标栏；分镜七项管理入口集中，元素图片生成/上传/确认沿用真实接口，批量操作先查看配置再提交。先同步PRD原型，后修改生产代码；范围、四主题证据与简明试用见[本轮验收记录](docs/engineering/compact-workspace-verification.md)。入口仍为5180，无新增付费调用、数据库迁移或公网部署，视觉效果待用户验收。

## 2026-09-11 公网版本更新

当前生产代码`e25c962`已部署到[原HTTPS入口](https://121.199.40.214/)，覆盖M3及最新分镜/元素交互增量，原网站登录信息不变。服务器11项目29媒体及凭据保留，本地数据未覆盖服务器。发布和回退证据见[服务器部署记录](docs/engineering/server-deployment.md)。部署通过不代表最终视觉或用户独立全链验收通过。
