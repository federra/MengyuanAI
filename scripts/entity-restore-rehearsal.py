"""Restore an existing backup into a disposable database and verify element workflow upgrade.
...
name = "shortfilm_restore_elements_" + uuid4().hex[:12]
...
env = {
    **os.environ,
    ...
    "SHORTFILM_STORAGE_ROOT": str(backup / "media"),
...
def inventory(conn, original):
    result = {}
    for table, names in original.items():
        query = sql.SQL(
...
try:
    ...
    with connect(isolated) as conn:
        original_columns = columns(conn)
        before = inventory(conn, original_columns)
    subprocess.run(
        [sys.executable, "-m", "alembic", "-c", "services/backend/alembic.ini", "upgrade", "head"],
        cwd=root,
        env=env,
        check=True,
    )
    with connect(isolated) as conn:
        after = inventory(conn, original_columns)
...
for p in r.json()['items']:
 ...
 for suffix in ['/stages/board','/finishing','/storyboard/import/assets','/entities','/reference-images','/media/reference-bindings']:
  response=c.get(base+suffix); assert response.status_code==200,(suffix,response.text)
assert c.get('/api/v1/library/assets').status_code==200
...
"""
    ...
    report = {
        "backup": str(backup),
        "migration": revision,
        "existingTables": before,
        "fileCount": len(rows),
        "existingRowsUnchanged": True,
        "mediaHashesUnchanged": True,
        "credentials": "not copied or read",
        "modelCalls": 0,
    }
    destination = root / "evidence/entity-restore.json"
    destination.parent.mkdir(exist_ok=True)
    destination.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n")
    print(
        f"Isolated restore/upgrade passed: {len(before)} existing tables, {len(rows)} media files; report {destination}"
    )
finally:
    with connect(admin, autocommit=True) as conn:
