...
## 三类元素管理修正检查点（2026-09-10）

当前工作树已补原子多元素保存、参考输入/选用输出区分、批量持久制图、镜头add/replace稳定绑定、引用阻删/历史、显式库副本及跨项目采用，migration为20260910_entity_workflow，尚未应用正式库。工程与四主题只读检查已完成阶段性验证，真实保存重载需先完成本地升级；用户视觉验收未通过。不调用付费模型、不推送/合并/部署。事实与测试分轮边界见docs/engineering/entity-management-verification.md；可审查停写/新备份/恢复/迁移/重启方案见docs/engineering/entity-management-upgrade.md。音色当前复用具名角色配置和供应商ID，独立声音资产CRUD仍是差距，不将本轮写成PRD08全部完成。

2026-09-10后续：用户明确批准后，三类元素管理正式本地库已迁移至20260910_entity_workflow并重启。新备份本工作树`.local/backups/20260910T082848960025Z`，13项目36文件恢复通过；37旧表原字段与全部媒体保持，既有MP4哈希不变。新增工程验收项目58943e81-1322-433b-83a8-7ee97f8cb46d验证三类元素真实浏览器统一保存/重载，现14项目，0模型任务；原13项目未改内容，2条历史unknown未重试。用户视觉接受及已列明声音资产差距仍待处理，详见entity-management-verification.md末节。

## 分镜参考图标尺增量（2026-09-10，未关闭验收）
当前集成工作区`.local/ui-v12-v13`按最新用户参考图调整为弹性主内容、台词麦克风弹窗、竖排64px图片槽、字段显隐和三类实体七列整表；新增一次分镜响应的实体目录契约及声音资产版本资源，复用既有PG表，无新增迁移。233项后端通过；真实5180只读四主题/三视口检查与新版4174原型检查通过。API/Worker仍待按具体方案重载；时长继承选择、真实新目录模型质量和最终用户视觉验收待定，不得写成整个产品完成。事实及后续浏览器结果见`docs/engineering/reference-ruler-verification.md`。

后续用户批准重载：最新备份`.local/backups/20260910T142519132033Z`已包含14项目36媒体及589源码文件；隔离恢复通过。API/Worker/UI已重启，40表摘要、媒体与原MP4下载哈希不变，四主题12组真实只读页面复核通过。无新迁移、付费调用或历史unknown重试；声音资产类型已载入，目录解析适用于后续新分镜任务，旧项目不回填。用户视觉接受、真实目录质量与时长继承决策仍未完成，不能把工程验证写成用户验收。

## 项目与分镜细化（2026-09-10）
最新用户要求已同步PRD与原型，生产增量包括项目菜单软删除/恢复、故事至导出标题项目摘要、分镜表增高/表头居中/图片小标签/32px台词控件、三类管理上传清除对齐及直接生图。复用PG表与持久任务，无新迁移、无付费调用；实际15项目36文件备份恢复及本地重载已完成，详细证据与最终测试见`docs/engineering/project-board-polish-verification.md`。用户已持续授权不涉及费用的必要操作，不再反复请求批准；新增付费调用仍须明确确认。用户视觉验收未通过。

2026-09-11保存禁用纠错：角色生图结果仅预览、未进入选用草稿导致保存按钮禁用；已修前端将当前有效未选用图纳入待保存草稿，保留手动选图/清除/视觉修改与冲突保护。20项相关页面回归和构建通过，无付费调用/数据库迁移/后端重启。见docs/engineering/generated-image-save-fix.md。

2026-09-11图片引用增量：三类管理缩略图可开关完整图片预览，分镜角标显示实际文件数（含0/1）；新媒体成功事务按显式元素/镜头ID自动引用，保留待确认、过期和重复完成保护。用户明确授权将8848旧版“护目镜/珠峰峰顶下方/珠峰北坡”3张图复用到当前目录，已保存新版本并覆盖6处引用，不凭同名自动归并其他项。239后端、22相关页面与四主题实际预览通过，15项目43媒体升级前备份恢复完成，新增费用0。详见docs/engineering/image-preview-auto-reference.md；最终视觉与M3用户试用仍待验收。

## 风格及具名图片引用（2026-09-11）

本地已补入现代电影/古风写意/清新动画三份可编辑版本化模板；@选择图片显示名称、存储稳定ID，AI目录发布显式绑定，风格正文沿用分镜及视频快照。241后端和6相关页面通过；四主题实页窗口检查及15项目43文件隔离恢复通过，无新增付费调用。时长与生成按钮仅检查未改规则；8848当前分镜未确认，部分时长可能超模型范围。真实新成片风格与用户UI验收未计通过。详见docs/engineering/style-mentions-verification.md。

## 角色括号链接与完整图片（2026-09-11）

提示词现在可点击@名称预览对应稳定ID图片，角色引用显示在人名后的括号中；可原位编辑、关闭预览保留草稿，旧历史不批量改写。242后端/5相关页面与四主题实页检查通过，15项目43文件备份恢复通过。8848第4版已保存但未确认，批量站位图/单镜视频禁用原因已解释，本轮未改门槛或代确认。详见docs/engineering/inline-image-links-verification.md；无付费调用，用户视觉验收仍待。

## 2026-09-11 已授权合并及公网更新

用户明确授权后，集成代码e25c962已快进合并main、推送GitHub并覆盖原服务器HTTPS位置。242后端/110页面通过；服务器隔离恢复迁移、正式旧行摘要、29媒体HTTPS哈希通过，11项目及原凭据保留。新release /opt/mengyuanai/releases/e25c962；成功备份/opt/mengyuanai/backups/20260911T065907。未同步本地数据库或新媒体，无模型调用；最终视觉与全链用户验收仍待。最新事实见docs/engineering/server-deployment.md，本条覆盖历史“未部署”状态，不抹去历史验收边界。
