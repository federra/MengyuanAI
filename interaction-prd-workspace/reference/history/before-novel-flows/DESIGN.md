...
> G3 已确认设计规范 v1.0｜当前 MVP：一句话到短片、故事到短片。G2 已按用户范围补充通过；用户已明确“同意”本规范，现制作组件与状态展示页；G3 整体仍待展示页审核。所有色值、尺寸为拟定规范，不冒充截图原始 CSS。
...
| 用途 | 字号 / 行高 | 使用规则 |
| --- | --- | --- |
| 页面标题 | 28px / 40px | 一页一个主要标题 |
| 区块标题 | 20px / 30px | 剧本、分镜、问题列表等区域 |
| 正文、表格内容、输入、按钮 | 16px / 26px | 剧本、审核意见、提示词不降级成辅助字 |
| 表头、辅助说明、状态标签 | 14px / 22px | 只承载次要信息；本产品不采用更小字号 |
| 长文编辑 | 16px / 28px | 剧本、故事、提示词及分镜长描述 |

全局间距以 4px 为最小步长，常用 8/12/16/24/32px。按钮、输入和下拉高度至少 44px，水平内边距 16px；图标按钮至少 44 × 44px 并有可访问名称。表格单元格建议内边距 12–16px，长内容顶对齐，不能靠固定短行高裁切。控件圆角 6px、面板 8px、标签 4px，边框 1px；浮层仅使用轻阴影辅助层级。

焦点轮廓使用 accent 色，建议 2px 实线及 2px 外偏移；聚焦不改变布局。错误输入同时说明字段和修复动作。200% 浏览器缩放下允许合理滚动、折叠侧栏和展开编辑，关键操作必须可访问；不得将整个页面缩小来抵消用户放大。

## 四、页面框架与交互视觉

产品原型使用 PC 固定视口 1440 × 900。页框建议顶部工具栏 64px、左侧阶段导航 208px、内容区外边距 24px、主区域间距 16–24px。默认竖屏是视频画幅，不是手机端原型。窗口不足时采用区域滚动和可折叠导航；具体模块保证长文编辑与媒体预览均能访问。

首页只有“一句话创作”“故事改编”两个入口，保留作品创建、保存和继续编辑。模型/提示词配置作为必要工具入口；不增加账号、权限及其他非主线导航。顶部统一放项目名称、保存状态与主题切换，主要运行按钮就近放在当前阶段。
...
本次请审核：双主题配色、主题切换与偏好保持、16px 正文及 14px 辅助字下限、控件尺寸与 PC 视口、页面框架及状态视觉原则。以上是具体设计提案，用户要求的双主题与清晰字号本身已确认。
...
本轮视觉补充：采用用户新增分镜截图的六列表格结构（序号、台词、角色/场景/道具、提示词、视频、操作），不通过缩小字号塞入一屏；表格局部滚动，剧本使用独立页面。提示词配置、系统配置与风格模板按钮常驻顶部，深浅主题共享布局。参考图片：assets/user-storyboard-reference.png。
