...
export function polishStudio(){
  ...
  motionContext=gsap.context(()=>{
    if(first){gsap.from('.studio .page-heading',{y:8,opacity:.6,duration:.4,clearProps:'transform,opacity'});first=false;}
    document.querySelectorAll('.topic-card').forEach((card,i)=>{
      const key=card.dataset.topic;if(seen.has(key))return;seen.add(key);
      ...
      gsap.from(card.querySelector('.topic-art'),{scale:.96,opacity:.7,duration:.65,clearProps:'transform,opacity',scrollTrigger:{trigger:card,start:'top 98%',once:true}});
    });
  });
}
