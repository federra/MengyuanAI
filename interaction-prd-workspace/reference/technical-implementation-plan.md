...
| `POST /projects/{id}/story-batches`；`POST /projects/{id}/stories/import-txt` | 一句话生成返回长任务；TXT同步确定性导入，返回StoriesOut并保留全文，不创建AI提炼任务 |
...
R4 的 P01–P28 使用独立 interactionKey 绑定提示词/模型。首先接入 P01–P08、P11–P13、P15、P17、P18、P25、P27、P28；TXT入口不调用P26，P26保留为可选增强。P10 资产提取先可人工维护，其他增强按 R4 推进。写作、剧本和分镜 Skill 作为带版本的指令包导入，校验变量与输出 Schema，不执行任意脚本。质检模型可独立配置，不要求与创作模型相同。
...
## 创意/故事精简与TXT接口增量 · 2026-09-10

本增量不新增数据库schema，不自动质检，不调用付费模型导入TXT。创建成功直达创意；创意/故事停输700ms自动保存，状态可见，草稿以项目/候选/源版本隔离保存；切换、刷新、失败与409冲突保留草稿，冲突不静默覆盖。提交生成前取消防抖等待并保存最新正文，再使用返回的精确版本。生成剧本的选定故事与Job/Outbox入队在同一事务，失败不得留下仅选定却未入队的状态。

`POST /projects/{id}/stories/import-txt`携带`Idempotency-Key`，JSON为`filename/text/expected_story_version_id`，同步返回带`source_mode`的`StoriesOut`，替代旧表中的story-imports规划。校验.txt、非空、UTF-8正文≤1,048,576字节，客户端fatal解码拒绝非法UTF-8；可接受BOM。服务端校验项目归属、当前故事版本和幂等摘要，在事务中追加导入版本、保留旧候选/版本并切换当前单卡，复用现有内容结构。同键同请求返回同一结果；同键异请求或源版本冲突拒绝，客户端保留草稿。

标题取文件名去扩展名，摘录取原文首句，正文完整保存，不调用P26。超过50,000字符仍可导入，但后续剧本生成、故事质检及导演建议等AI调用在入队前明确阻止，提示先选改编范围；不静默截断。后续AI故事批次仅在成功写入后将source_mode切回候选模式，失败保持TXT单卡。故事质检放右侧导演助手、仅手动触发；更多方案放左侧卡片下。

增量验收覆盖创建跳转、字段移除、700ms保存/生成前保存、失败/冲突草稿、原子选定入队、幂等重试与冲突、空文件/扩展名/UTF8/1MB边界、全文单卡与历史保留、50k边界及AI拒绝、后续成功AI批恢复候选。原型只证明交互示例，生产验证使用隔离环境，不操作生产数据或新增付费。


新 board.generate 任务追加应用约束：source.text 完整剧本正文是唯一叙事依据，完整保留场景、人物动作与台词；scenes 仅为派生结构，estimatedSeconds 仅供参考，不得当作固定时长或删减依据。提交前保存返回版本与任务source版本一致；旧模板、历史任务及快照不重写。


## 2026-09-10 紧凑工作区增量

本轮先同步PRD/Demo，再开展生产增量。前端只持久化导航显示偏好，不用浏览器替代正式业务存储；元素列表与选中编辑复用现有稳定ID、保存及图片服务，批量确认清单复用既有提交接口。无新增数据库迁移。生产必须核对真实图片管理入口、批量提交防重、草稿保留、四主题与U01–U08适用项；此原型完成记录不表示生产或最终UI已验收。

紧凑工具栏批量命令补充：客户端按项目/分镜版本持久保留未知请求正文与幂等身份；结果未确定时不得通过改表现新建同项请求。受理成功与列表刷新成功分开判断，已受理目标重开不重复提交；原任务失败走原任务恢复。生成设置异步回执只更新仍在查看的同一项目，不能切换用户当前项目；图片/任务首次读取完成前不显示假空状态或开放生成。
