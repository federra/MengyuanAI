...
## 2026-09-10 三类元素管理完整交互增量

此增量补齐上述紧凑窗口的业务操作，需迁移至`20260910_entity_workflow`；不以早期“无新增迁移”描述覆盖本次事实。`EntityVersion`分离input_file_id/output_file_id及资产库来源；全量草稿以`PUT /projects/{pid}/entities/batch`单事务、项目锁、实体revision和分镜基准保存，客户端稳定UUID及持久幂等记录防止丢回执重复创建。仅显式LineBinding传播角色名/音色，不按姓名猜历史关联。所有历史内容和任务快照保留。

`POST /entities/{eid}/shot-bindings`携带镜头/分镜/实体版本，add增加稳定ref、replace保留首同类ref；绑定追加版本，可在无图时保留占位但禁止生成视频/导出。显式关联实体变更参与镜头输入比较；同一文件被其他元素确认不能绕过本元素过期。删除仅归档，无当前镜头/台词关联才允许，历史API保留。导入描述的既有entity_version关联可复用，不合并同名历史实体。

`POST /entities/image-batches`原子入队，每项继承现有真实制图接口及持久恢复，服务端自动加入当前已保存输入图，冻结图片字节摘要/描述/三视图/配置；同元素已有处理中任务时拒绝重新起批。预览不生成，提交未知时重试原body/key。图片上传、选用与人工确认分开；AI质检非门槛。

`library_assets`为显式存库的不可变副本，每次存库生成独立asset ID/version=1；采用指定版本创建目标项目实体及独立MediaFile，保存来源，不自动覆盖其他项目。`library_adoptions`及`entity_commands`保存重放凭据（非供应商密钥）。现阶段不提供修改已有库副本或自动升级接口。备份扫描项目媒体及独立library文件，校验哈希；升级前停写备份、隔离恢复后才切本地服务。本轮工程与UI证据见`docs/engineering/entity-management-verification.md`，最终用户视觉接受单独记录。
...
新增项目DELETE与恢复接口，复用status及JSON配置保存软删除前状态，无schema迁移。持项目行锁与revision防并发；有活动或unknown任务返回409；列表统计排除deleted，普通项目入口禁止访问已删除项目。直接图片提交复用持久image-batches，不改供应商契约及版本失效。标题元数据来自项目/idea/配置API，视觉偏好不写业务内容。


## 2026-09-11 图片预览与自动引用

媒体完成事务按快照实体ID/镜头ID新增真实引用；来源过期或取消只归档不绑定，重复完成幂等。角色、场景、道具共同代码路径；站位图追加分镜新版且不改提示词，避免自身过期。历史不同ID复用仅按用户明确映射，不根据名称自动迁移。
