...
创意助手不提供方法选择，故事助手与创意主工作区共用写作方法；故事生成剧本与剧本助手共用剧本方法；剧本生成分镜与分镜助手共用分镜生成方法。各自interactionKey仍独立，任务冻结本次方法版本。质检及修复提示词应描述问题、依据、修改建议，不输出强迫用户修改或禁止推进的产品指令。AI报告不是工作流授权结果；报告未采用时原结论保留，由应用记录用户选择继续。尾帧通过稳定refId与图片输入序列对应，提示词不得虚构已存在的前镜图。

## V9 具名方法契约

具名方法绑定单个Skill或提示词模板，以resourceId、resourceVersion、kind、stage定位，禁止根据显示名或类型猜选。故事/剧本/分镜主生成及对应修改记录所选方法和本次实际正文；创意导演对话P25不挂写作方法，仍使用自己的交互模板。模板库为配置中心与资产库的同一数据源，项目覆盖独立保存。模型请求组合操作模板、选中方法、源文本、用户要求和报告上下文；instruction为用户本轮额外要求，不递归拼接模板自身。
