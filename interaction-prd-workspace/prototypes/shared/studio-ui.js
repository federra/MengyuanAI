...
export function methodMarkup(key){const m=C.s[key];return `<div class="method-select"><label>${key==='writingMethod'?'写作':key==='boardMethod'?'分镜生成':'剧本'}方式<select data-method-mode="${key}"><option value="skill" ${m.mode==='skill'?'selected':''}>${key==='writingMethod'?'写作':key==='boardMethod'?'分镜生成':'剧本'} skill</option><option value="prompt" ${m.mode==='prompt'?'selected':''}>${key==='writingMethod'?'写作':key==='boardMethod'?'分镜生成':'剧本'}提示词</option></select></label><details><summary>查看 / 编辑指令</summary><label class="field">${m.mode==='skill'?'Skill 内容（示例，可自定义）':'本次提示词'}<textarea data-method-text="${key}">${esc(m.text)}</textarea></label><p class="helper">选择和内容会随任务保存；此处未读取或运行机器上的真实 skill。</p></details></div>`;}
export function bindMethods(root=document){root.querySelectorAll('[data-method-mode]').forEach(e=>e.onchange=()=>{const m=C.s[e.dataset.methodMode];m.mode=e.value;m.version++;save();C.render()});root.querySelectorAll('[data-method-text]').forEach(e=>e.oninput=()=>{const m=C.s[e.dataset.methodText];m.text=e.value;m.version++;save()});}
