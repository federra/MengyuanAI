...
import { mediaRequest, useMedia, ShotPosition } from "./MediaWorkbench";
  ...
  return (
    <div className="shot-reference-grid">
      ...
      {(["characters", "scenes", "props", "positions"] as const).map((kind) => {
        ...
        const linkedFiles = ids.map(id => {
          const ref = m.references.find(r => r.shot_id === shot.id && r.ref_id === id);
          return ref ? ref.file_id : id;
        }).filter((id): id is string => !!id && images.some(f => f.id === id));
        return (
          <section className="reference-window" key={kind}>
            <button
              className="reference-square"
              aria-label={label + "图片"}
              title={label}
              onClick={() => {
                setActiveKind(kind);
            ...
            >
              <span className="reference-kind-label">{label}图</span>
              {linkedFiles[0] ? <img src={`/api/v1/projects/${m.pid}/files/${linkedFiles[0]}`} alt={label}/> : "+"}
              <span className="reference-count" aria-label={`${label}图片数量`}>{linkedFiles.length}</span>
            </button>
            {activeKind === kind && (
              <dialog
                ref={dialog}
                    ...
                    <ShotPosition
                      ...
                      onBind={(id) =>
                        onChange({
                          ...shot.refs,
                          positions: [
                            ...new Set([...(shot.refs.positions || []), id]),
                          ],
                        })
                      }
                    />
