export function projectMentions(raw: string, names: Map<string, string>) {
...
export function editMentions(
  ...
  let prefix = 0,
    suffix = 0;
  while (
    prefix < old.text.length &&
    prefix < next.length &&
    old.text[prefix] === next[prefix]
  )
    prefix++;
  while (
    suffix < old.text.length - prefix &&
...
export function inlineCharacterMentions(
  raw: string,
  ...
  if (!characters.size) return raw;
  const pattern = new RegExp(
    [...new Set(characters.values())]
      .filter(Boolean)
  ...
  const eligible = new Map(
    [...characters].filter(
      ([id, name]) =>
        counts.get(name) === 1 && present.has(name) && raw.includes(`@[${id}]`),
    ),
  );
  let result = raw;
  ...
  return result
    ...
    .map((part) =>
      part.startsWith("@[")
        ? part
        : part.replace(pattern, (name) =>
            byName.has(name) ? `${name}（@[${byName.get(name)}]）` : name,
          ),
    )
    .join("")
    .trimEnd();
